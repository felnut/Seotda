...
function MiniCard({ cardId, alt }: { cardId: string; alt: string }) {
  return (
    <div className="relative h-16 w-10.5 shrink-0 overflow-hidden rounded-md border border-white/10 shadow-sm">
      <Image src={`/card/${cardId}.png`} alt={alt} fill sizes="42px" className="object-cover" />
    </div>
  );
}
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col px-4 py-10 sm:py-14">
      ...
      <p className="mb-8 text-[14.5px] leading-relaxed text-zinc-400">
        섯다는 두 장(때로는 세 장 중 두 장)의 카드가 이루는 월(月) 숫자
        조합, 즉 족보로 승부를 겨룹니다. 아래는 특수 족보와 일반 족보를
        순위 높은 순으로 정리한 표입니다. 게임 화면에서도 같은 내용을
        가이드 패널로 바로 확인할 수 있지만, 미리 훑어보고 들어가면
        처음 하는 분도 훨씬 수월하게 즐길 수 있습니다.
      </p>
      ...
      <section className="mb-8">
        ...
        <p className="mb-4 text-[14.5px] leading-relaxed text-zinc-400">
          조건이 맞으면 순위표와 무관하게 즉시 무승부·재경기가 되거나
          승패가 뒤바뀌는 조합입니다. 상대의 패를 미처 예상하지 못했을 때
          가장 짜릿한 반전을 만들어냅니다.
        </p>
      ...
      <section>
        ...
        <p className="mb-4 text-[14.5px] leading-relaxed text-zinc-400">
          특수 족보에 해당하지 않으면, 아래 순위표에 따라 더 높은 족보를
          가진 쪽이 이깁니다. 가장 높은 광땡부터 가장 낮은 망통까지, 총{" "}
