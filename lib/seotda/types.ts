...
export interface Player {
  ...
  isSpectator: boolean;
  // 다음 start() 호출(다음 판 시작) 때 isSpectator가 풀리며 실제 참가자로
  // 전환되도록 예약됐는지. 진행 중인 판 도중 새로 들어온 플레이어는 이
  // 값이 false인 채로 관전만 시작하며(addPlayer 참고), requestJoinNextRound()로
  // 스스로 "경기 참여"를 눌러야만 true가 된다 — 자동으로 참여시키지 않는다.
